 ...
 *
 * Call [appModule] with a populated [Properties] instance (loaded from config.properties)
 * before starting Koin. All singletons are created lazily on first use.
 *
 * Bindings:
 * - [DocumentLoader]  → [PDFBoxDocumentLoader]
 * - [Chunker]         → [FixedSizeChunker] (500 chars, 50-char overlap)
 * - [Embedder]        → [AllMiniLmL6V2Embedder]  (local ONNX, no API key needed)
 * - [VectorStore]     → [PgVectorStore] with [InMemoryVectorStore] fallback
 * - [LlmClient]       → [GroqApiClient] or [GeminiApiClient] based on `llm.provider` config
 */
fun appModule(properties: Properties) = module {
    ...
    // DatabaseManager handles the PgVector → InMemoryVectorStore fallback internally
    single<VectorStore> { DatabaseManager.setupVectorStore(get()) }

    single { DataIngestionService(get(), get(), get(), get()) }

    // ── LLM client — provider selected from config ──────────────
    single<LlmClient> {
